...
CSS = """
        /* ─────────────────────────────────────────────────────────────
           ...
           ───────────────────────────────────────────────────────────── */
        .topbar {
            /* Not sticky. Apple's bar stays, but it earns that with
               translucency and a backdrop blur — the effect only reads
               when content slides under it. A bar that scrolls away has
               nothing to blur, so it wants an opaque surface and a
               defined edge instead, which is what this is. */
        ...
        /* Lay the nav out explicitly rather than inheriting Pico's
           cascade — the wrapper div changed which rules win, and the
           bar must not depend on that. This reproduces the original
           layout: wordmark left, links right, one row. */
        .topbar nav {
            display: flex;
            align-items: center;
        ...
        .topbar nav li { list-style: none; margin: 0; }
        .topbar nav li::before { content: none; }
